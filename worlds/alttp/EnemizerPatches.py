...
def _randomize_enemy_damage(rom: "LocalRom", rng: random.Random, allow_zero_damage: bool) -> None:
    for sprite_id in range(0xF3):
        ...
        if not allow_zero_damage and new_damage == 2:
            continue
        try:
            high_nibble = rom.read_byte(damage_address) & 0xF0
        except RuntimeError:
            high_nibble = VANILLA_ENEMY_DAMAGE_TABLE_HIGH_NIBBLES[sprite_id]
        rom.write_byte(damage_address, high_nibble | new_damage)
