...
class PokemonRedBlueWorld(World):
    ...
    def fill_hook(self, progitempool, usefulitempool, filleritempool, fill_locations):
        if not self.options.badgesanity:
            ...
            if not self.options.door_shuffle not in ("off", "simple"):
    ...
    def pre_fill(self) -> None:
        ...
        def intervene(move, test_state):
            ...
            if self.options.randomize_wild_pokemon and viable_mons:
                ...
                replace_mon = self.random.choice(viable_mons)
                replace_slot = self.random.choice([slot for slot in accessible_slots if slot.item.name
                                                              == placed_mon])
                if self.options.area_1_to_1_mapping:
        ...
        while True:
            ...
            # as you will require cut to access celadon gyn
                    ...
                    (self.options.accessibility != "minimal" or ((not
                     self.options.badgesanity) and max(
                     self.options.elite_four_badges_condition.value,
                     self.options.route_22_gate_condition.value,
                     self.options.victory_road_condition.value)
                     > 7) or (self.options.door_shuffle not in ("off", "simple")))):
                intervene_move = "Cut"
            elif ((not logic.can_learn_hm(test_state, "Flash", self.player))
                   and self.options.dark_rock_tunnel_logic
                   and (self.options.accessibility != "minimal"
            ...
            # considered in door shuffle.
                    ...
                    and self.options.door_shuffle not in
