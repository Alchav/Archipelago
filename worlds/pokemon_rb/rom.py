...
def resolve_rom_address(world, address, offset=0):
    if address is None:
        return None
    if isinstance(address, str):
        return world.rom_addresses[address] + offset
    return address + offset


def resolve_rom_addresses(world, address, offset=0):
    if not isinstance(address, list):
        address = [address]
    return [resolve_rom_address(world, single_address, offset) for single_address in address]
...
class PokemonRedProcedurePatch(APProcedurePatch, APTokenMixin):
    ...
    def get_source_data(cls) -> bytes:
        ...
        with open(PokemonRedWorld.settings.red_rom_file, "rb") as infile:
...
class PokemonBlueProcedurePatch(APProcedurePatch, APTokenMixin):
    ...
    @classmethod
    def get_source_data(cls) -> bytes:
        from . import PokemonBlueWorld
        with open(PokemonBlueWorld.settings.blue_rom_file, "rb") as infile:
            base_rom_bytes = bytes(infile.read())

        return base_rom_bytes


class PokemonYellowProcedurePatch(APProcedurePatch, APTokenMixin):
    game = "Pokemon Yellow"
    hash = "d9290db87b1f0a23b89f99ee4469e34b"
    patch_file_ending = ".apyellow"
    ...
    @classmethod
    def get_source_data(cls) -> bytes:
        from . import PokemonYellowWorld
        with open(PokemonYellowWorld.settings.yellow_rom_file, "rb") as infile:
            base_rom_bytes = bytes(infile.read())

        return base_rom_bytes


def write_quizzes(world: "PokemonRedWorld | PokemonBlueWorld | PokemonYellowWorld",
                  patch: PokemonBlueProcedurePatch | PokemonRedProcedurePatch | PokemonYellowProcedurePatch):
    random = world.random
    trade_quizzes = [trade for trade in world.trade_data if trade.quiz_text]

    def get_quiz(q: int, a: int):
        if q == 0:
            trade = random.choice(trade_quizzes)
            mon = world.trade_mons[trade.rom_address]
            text = trade.quiz_text
            if not a:
                answers.append(0)
                old_mon = mon
                while old_mon == mon:
    ...
    for i, quiz in enumerate(["A", "B", "C", "D", "E", "F"]):
        patch.write_token(APTokenTypes.WRITE, world.rom_addresses[f"Quiz_Answer_{quiz}"], bytes([int(not answers[i]) << 4 | (i + 1)]))
        patch.write_token(APTokenTypes.WRITE, world.rom_addresses[f"Text_Quiz_{quiz}"], bytes(question_texts[i]))


def generate_output(world: "PokemonRedWorld | PokemonBlueWorld | PokemonYellowWorld", output_directory: str):

    patch = world.patch(player=world.player, player_name=world.player_name)
    basepatch_name = f"basepatch_{world.game.split(' ')[1].lower()}.bsdiff4"
    patch.write_file("base_patch.bsdiff4", pkgutil.get_data(__name__, basepatch_name))
    ...
    if paths:
        ...
        write_bytes(world.rom_addresses["Path_Pallet_Player"], paths[1])

    if pallet_connections["Rival's House"] == "Player's House 1F":
        write_bytes(world.rom_addresses["Pallet_Fly_Coords"], [0x2F, 0xC7, 0x06, 0x0D, 0x00, 0x01])
    elif pallet_connections["Oak's Lab"] == "Player's House 1F":
        write_bytes(world.rom_addresses["Pallet_Fly_Coords"], [0x5F, 0xC7, 0x0C, 0x0C, 0x00, 0x00])
        ...
        for entrance in region.exits:
            if isinstance(entrance, PokemonRBWarp):
                ...
                for i, warp_id in enumerate(warp_ids):
                    address = world.rom_addresses[entrance.address]
    ...
    if world.options.door_shuffle == "simple":
        for (entrance, _, _, map_coords_entries, map_name, _) in town_map_coords.values():
            ...
            for map_coord_entry in map_coords_entries:
                write_bytes(world.rom_addresses["Town_Map_Coords"] + (map_coord_entry * 4) + 1, (y << 4) | x)
            write_bytes(world.rom_addresses["Town_Map_Order"] + map_order_entry, map_ids[map_name])
    ...
    for i, gym_leader in enumerate(("Pewter Gym - Brock TM", "Cerulean Gym - Misty TM",
        ...
        if item_name.startswith("TM"):
            try:
                ...
                write_bytes(world.rom_addresses["Gym_Leader_Moves"] + (2 * i), move)
            except KeyError:
                pass

    def set_trade_mon(address, loc):
        mon = world.multiworld.get_location(loc, world.player).item.name
        write_bytes(world.rom_addresses[address], poke_data.pokemon_data[mon]["id"])
        world.trade_mons[address] = mon

    for trade in world.trade_data:
        set_trade_mon(trade.rom_address, trade.source_location)

    write_bytes(world.rom_addresses["Fly_Location"], world.fly_map_code)
    write_bytes(world.rom_addresses["Map_Fly_Location"], world.town_map_fly_map_code)

    if world.options.fix_combat_bugs:
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs"], 1)
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_Focus_Energy"],  0x28)  # jr z
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_HP_Drain_Dream_Eater"], 0x1A)  # ld a, (de)
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_PP_Restore"], 0xe6)  # and a, direct
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_PP_Restore"] + 1, 0b0011111)
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_Struggle"], 0xe6)  # and a, direct
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_Struggle"] + 1, 0x3f)
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_Dig_Fly"], 0b10001100)
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_Heal_Effect"], 0x20)  # jr nz,
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_Heal_Effect"] + 1, 5)  # 5 bytes ahead
        write_bytes(world.rom_addresses["Option_Fix_Combat_Bugs_Heal_Stat_Modifiers"], 1)

    if world.options.poke_doll_skip == "in_logic":
        write_bytes(world.rom_addresses["Option_Silph_Scope_Skip"], 0x00)      # nop
        write_bytes(world.rom_addresses["Option_Silph_Scope_Skip"] + 1, 0x00)  # nop
        write_bytes(world.rom_addresses["Option_Silph_Scope_Skip"] + 2, 0x00)  # nop

    if world.options.bicycle_gate_skips == "patched":
        write_bytes(world.rom_addresses["Option_Route_16_Gate_Fix"], 0x00)      # nop
        write_bytes(world.rom_addresses["Option_Route_16_Gate_Fix"] + 1, 0x00)  # nop
        write_bytes(world.rom_addresses["Option_Route_18_Gate_Fix"], 0x00)      # nop
        write_bytes(world.rom_addresses["Option_Route_18_Gate_Fix"] + 1, 0x00)  # nop

    if world.options.door_shuffle:
        write_bytes(world.rom_addresses["Entrance_Shuffle_Fuji_Warp"], 1)  # prevent warping to Fuji's House from Pokemon Tower 7F

    if world.options.all_elevators_locked:
        write_bytes(world.rom_addresses["Option_Locked_Elevator_Celadon"], 0x20)  # jr nz
        write_bytes(world.rom_addresses["Option_Locked_Elevator_Silph"], 0x20)    # jr nz

    if world.options.tea:
        write_bytes(world.rom_addresses["Option_Tea"], 1)
        write_bytes(world.rom_addresses["Guard_Drink_List"], 0x54)
        write_bytes(world.rom_addresses["Guard_Drink_List"] + 1, 0)
        write_bytes(world.rom_addresses["Guard_Drink_List"] + 2, 0)
        write_bytes(world.rom_addresses["Text_Saffron_Gate"],
                    encode_text("<LINE>Gee, I have the<CONT>worst caffeine<CONT>headache though."
                                "<PARA>Oh wait there,<LINE>the road's closed.<DONE>"))

        write_bytes(world.rom_addresses["Tea_Key_Item"], 0x28)  #  jr .z

    write_bytes(world.rom_addresses["Fossils_Needed_For_Second_Item"], world.options.second_fossil_check_condition.value)

    write_bytes(world.rom_addresses["Option_Lose_Money"], int(not world.options.lose_money_on_blackout.value))

    if world.options.extra_key_items:
        write_bytes(world.rom_addresses["Option_Extra_Key_Items_A"], 1)
        write_bytes(world.rom_addresses["Option_Extra_Key_Items_B"], 1)
        write_bytes(world.rom_addresses["Option_Extra_Key_Items_C"], 1)
        write_bytes(world.rom_addresses["Option_Extra_Key_Items_D"], 1)
    write_bytes(world.rom_addresses["Option_Split_Card_Key"], world.options.split_card_key.value)
    write_bytes(world.rom_addresses["Option_Blind_Trainers"], round(world.options.blind_trainers.value * 2.55))
    write_bytes(world.rom_addresses["Option_Cerulean_Cave_Badges"], world.options.cerulean_cave_badges_condition.value)
    write_bytes(world.rom_addresses["Option_Cerulean_Cave_Key_Items"], world.options.cerulean_cave_key_items_condition.total)
    write_bytes(world.rom_addresses["Text_Cerulean_Cave_Badges"], encode_text(str(world.options.cerulean_cave_badges_condition.value)))
    write_bytes(world.rom_addresses["Text_Cerulean_Cave_Key_Items"], encode_text(str(world.options.cerulean_cave_key_items_condition.total) + " key items."))
    write_bytes(world.rom_addresses["Option_Encounter_Minimum_Steps"], world.options.minimum_steps_between_encounters.value)
    write_bytes(world.rom_addresses["Option_Route23_Badges"], world.options.victory_road_condition.value)
    write_bytes(world.rom_addresses["Option_Victory_Road_Badges"], world.options.route_22_gate_condition.value)
    write_bytes(world.rom_addresses["Option_Elite_Four_Pokedex"], world.options.elite_four_pokedex_condition.total)
    write_bytes(world.rom_addresses["Option_Elite_Four_Key_Items"], world.options.elite_four_key_items_condition.total)
    write_bytes(world.rom_addresses["Option_Elite_Four_Badges"], world.options.elite_four_badges_condition.value)
    write_bytes(world.rom_addresses["Text_Elite_Four_Badges"], encode_text(str(world.options.elite_four_badges_condition.value)))
    write_bytes(world.rom_addresses["Text_Elite_Four_Key_Items"], encode_text(str(world.options.elite_four_key_items_condition.total) + " key items, and"))
    write_bytes(world.rom_addresses["Text_Elite_Four_Pokedex"], encode_text(str(world.options.elite_four_pokedex_condition.total) + " #MON"))
    write_bytes(world.rom_addresses["Trainer_Screen_Total_Key_Items"], encode_text(str(world.total_key_items), length=2))

    write_bytes(world.rom_addresses["Option_Viridian_Gym_Badges"], world.options.viridian_gym_condition.value)

    write_bytes(world.rom_addresses["Option_EXP_Modifier"], world.options.exp_modifier.value)
    if not world.options.split_exp:
        write_bytes(world.rom_addresses["Option_No_Split_EXP_A"], [0, 0, 20])  # nop, nop, jr nz
    ...
    if world.options.extra_key_items:
        for i in range(0, 4):
            write_bytes(world.rom_addresses["Option_Rock_Tunnel_Extra_Items"] + (i * 3), 0x15)
    if world.options.old_man == "open_viridian_city":
        write_bytes(world.rom_addresses["Option_Old_Man"], 0x11)
        write_bytes(world.rom_addresses["Option_Old_Man_Lying"], 0x15)
    ...
    if world.options.require_pokedex:
        write_bytes(world.rom_addresses["Require_Pokedex_A"], 1)
        write_bytes(world.rom_addresses["Require_Pokedex_B"], 1)
        write_bytes(world.rom_addresses["Require_Pokedex_C"], 1)
    else:
        write_bytes(world.rom_addresses["Require_Pokedex_D"], 0x18)  # jr
    if world.options.dexsanity:
        write_bytes(world.rom_addresses["Option_Dexsanity_A"], 1)
        write_bytes(world.rom_addresses["Option_Dexsanity_B"], 1)
    if world.options.all_pokemon_seen:
        write_bytes(world.rom_addresses["Option_Pokedex_Seen"], 1)
    money = str(world.options.starting_money.value).zfill(6)
    write_bytes(world.rom_addresses["Starting_Money_High"], int(money[:2], 16))
    write_bytes(world.rom_addresses["Starting_Money_Middle"], int(money[2:4], 16))
    write_bytes(world.rom_addresses["Starting_Money_Low"], int(money[4:], 16))
    write_bytes(world.rom_addresses["Text_Badges_Needed_Viridian_Gym"],
                encode_text(str(world.options.viridian_gym_condition.value))[0])
    write_bytes(world.rom_addresses["Text_Rt23_Badges_A"],
                encode_text(str(world.options.victory_road_condition.value))[0])
    write_bytes(world.rom_addresses["Text_Rt23_Badges_B"],
                encode_text(str(world.options.victory_road_condition.value))[0])
    write_bytes(world.rom_addresses["Text_Rt23_Badges_C"],
                encode_text(str(world.options.victory_road_condition.value))[0])
    write_bytes(world.rom_addresses["Text_Rt23_Badges_D"],
                encode_text(str(world.options.victory_road_condition.value))[0])
    write_bytes(world.rom_addresses["Text_Badges_Needed"],
                encode_text(str(world.options.elite_four_badges_condition.value))[0])
    write_bytes(world.rom_addresses["Text_Magikarp_Salesman"],
    ...
    if world.options.badges_needed_for_hm_moves.value == 0:
        for hm_move in poke_data.hm_moves:
            write_bytes(world.rom_addresses["HM_" + hm_move + "_Badge_a"], [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        ...
        for hm_move, badge in world.extra_badges.items():
            write_bytes(world.rom_addresses["HM_" + hm_move + "_Badge_b"], badge_codes[badge])
            move_text = hm_move
            if badge not in ["Marsh Badge", "Volcano Badge", "Earth Badge"]:
                move_text = ", " + move_text
            rom_address = world.rom_addresses["Badge_Text_" + badge.replace(" ", "_")]
        ...
        for badge in ["Marsh Badge", "Volcano Badge", "Earth Badge"]:
            if badge not in written_badges:
                write_bytes(world.rom_addresses["Badge_Text_" + badge.replace(" ", "_")], encode_text("Nothing"))

    type_loc = world.rom_addresses["Type_Chart"]
    for matchup in world.type_chart:
    ...
    if world.options.normalize_encounter_chances.value:
        ...
        for i, chance in enumerate(chances):
            write_bytes(world.rom_addresses["Encounter_Chances"] + (i * 2), chance)

    for mon, mon_data in world.local_poke_data.items():
        if mon == "Mew":
            address = world.rom_addresses["Base_Stats_Mew"]
        else:
            address = world.rom_addresses["Base_Stats"] + (28 * (mon_data["dex"] - 1))
    ...
    write_bytes(world.rom_addresses["Option_Aide_Rt15"], world.options.oaks_aide_rt_15.value)

    if world.options.safari_zone_normal_battles.value == 1:
        write_bytes(world.rom_addresses["Option_Safari_Zone_Battle_Type"], 255)

    if world.options.reusable_tms.value:
        write_bytes(world.rom_addresses["Option_Reusable_TMs"], 0xC9)

    write_bytes(world.rom_addresses["Option_Always_Half_STAB"], int(not world.options.same_type_attack_bonus.value))
    ...
    if world.options.better_shops:
        ...
        for shop in range(1, 11):
            write_bytes(world.rom_addresses[f"Shop{shop}"], shop_data)
    if world.options.stonesanity:
        write_bytes(world.rom_addresses["Shop_Stones"], [0xFE, 1, item_table["Poke Doll"].id, 0xFF])

    price = str(world.options.master_ball_price.value).zfill(6)
    price = [int(price[:2], 16), int(price[2:4], 16), int(price[4:], 16)]
    write_bytes(world.rom_addresses["Price_Master_Ball"], price)  # Money values in Red and Blue are weird
    ...
    for item, value in start_inventory.items():
        if item > 255:
            item -= 256
        write_bytes(world.rom_addresses["Start_Inventory"] + item, min(value, 255))
    ...
    write_bytes(world.rom_addresses["TM_Moves"], TM_IDs)

    if world.options.randomize_rock_tunnel:
        # seed = randomize_rock_tunnel(patch, world.random)
        write_bytes(world.rom_addresses["Text_Rock_Tunnel_Sign"], encode_text(f"SEED: <LINE>{world.rock_tunnel_seed}"))
        patch.write_token(APTokenTypes.WRITE, world.rom_addresses["Map_Rock_Tunnel1F"], world.rock_tunnel_1f_data)
        patch.write_token(APTokenTypes.WRITE, world.rom_addresses["Map_Rock_TunnelB1F"], world.rock_tunnel_b1f_data)
    ...
    if world.game == "Pokemon Yellow":
        starter_ids.append(poke_data.pokemon_data["Pikachu"]["id"])
    else:
        ...
        for mon in range(0, 16):
            write_bytes(world.rom_addresses["Title_Mons"] + mon, mons.pop())

    write_bytes(world.rom_addresses["Title_Seed"], encode_text(world.multiworld.seed_name[-20:], 20, True))

    slot_name = world.multiworld.player_name[world.player]
    slot_name.replace("@", " ")
    slot_name.replace("<", " ")
    slot_name.replace(">", " ")
    write_bytes(world.rom_addresses["Title_Slot_Name"], encode_text(slot_name, 16, True, True))

    if world.trainer_name == "choose_in_game":
        write_bytes(world.rom_addresses["Skip_Player_Name"], 0)
    else:
        write_bytes(world.rom_addresses["Player_Name"], world.trainer_name)
    if world.rival_name == "choose_in_game":
        write_bytes(world.rom_addresses["Skip_Rival_Name"], 0)
    else:
    ...
    if world.options.text_speed.value == 0:
        ...
        write_bytes(world.rom_addresses["Option_Remove_Text_Delay"], [0, 0, 0])
    ...
    if "WTW" in world.options.debug_options.value:
        write_bytes(world.rom_addresses["Debug_WTW"], 0x18)
    ...
    for location in world.multiworld.get_locations(world.player):
        if location.party_data:
            for party in location.party_data:
                if not isinstance(party["party_address"], list):
                    addresses = [world.rom_addresses[party["party_address"]]]
                    parties = [party["party"]]
                else:
                    addresses = [world.rom_addresses[address] for address in party["party_address"]]
                    parties = party["party"]
                levels = party["level"]
        ...
        if location.item and location.item.player == world.player:
            if location.rom_address:
                for address in resolve_rom_addresses(world, location.rom_address, location.address_offset):
                ...
                if location.level:
                    for address in resolve_rom_addresses(world, location.level_address,
                                                         location.level_address_offset):
                        write_bytes(address, location.level)
