...
def get_item_spheres(multiworld: MultiWorld, beaten_game_spheres=None, return_unreachables=True):
    #logging.info("Running get_item_spheres")
...
def distribute_items_restrictive(multiworld: MultiWorld,
    ...
    for item in itempool:
        if item.advancement:
            ...
            # if not multiworld.player_name[item.player].startswith("Auto"):
            if item.useful:
    ...
    #     add_rule(location, lambda state: False)
    ...
    if option in ("e", "r"):
        ...
        logging.info(f"Highest initial sphere: {highest_sphere}")
        while True:
            logging.info("Sort attempt begin")
            new_player_names = []
            end_list_player_names = []
            for i, player in enumerate(player_names):
                if game_spheres[player[0]] - i < highest_sphere:
                    logging.info(f"{player[1]} fits")
                    new_player_names.append(player)
                    highest_sphere += 1
                else:
                    logging.info(f"{player[1]} pushed to end of list")
            ...
            if new_player_names + end_list_player_names == player_names:
                if len(end_list_player_names) > 1:
                    ...
                    logging.info(f"{end_list_player_names} pushed to end of list, could not fit")
                player_names = new_player_names + end_list_player_names
                break
            else:
                player_names = new_player_names + end_list_player_names
        logging.info(f"Highest final sphere: {highest_sphere}")
        ...
        while True:
            if player_names_copy == player_names:
                for i, (player, player_name) in enumerate(player_names):
                    logging.info(f"{player_name} forced to early sphere: {i}")
            ...
            for player, player_name in player_names:
                ...
                if sphere_to_place > 0:
                    ...
                    logging.info(f"{player_name} sphere to place: {sphere_to_place}")
                    logging.info(f"Game's spheres: {game_spheres[player]}")
                    logging.info(f"New highest sphere: {game_spheres[player] + sphere_to_place}")
                    highest_sphere += 1
    ...
    for player, starting_sphere in starting_spheres_list:
        ...
        logging.info(f"Highest sphere: {len(spheres)}")
        logging.info(f"Adding {multiworld.player_name[player]} with {game_spheres[player]} spheres")
        logging.info(f"Placing {multiworld.player_name[player]} unlock item in Sphere {starting_sphere}, should bring to Sphere {starting_sphere + game_spheres[player]}")
        ...
        if location.item:
            ...
            logging.info(f"Placing Unlock for {multiworld.player_name[player]} in Sphere {starting_sphere} ~ {location.name} ~ {multiworld.player_name[location.player]} displacing {location.item.name}")
        else:
            logging.info(f"Placing Unlock for {multiworld.player_name[player]} in Sphere {starting_sphere} ~ {location.name} ~ {multiworld.player_name[location.player]} displacing nothing")
        multiworld.push_item(location, multiworld.worlds[1].create_item(f"Unlock {multiworld.player_name[player]}"))
        # location.item =
        # location.item.location = location
            # logging.info(f"didn't reach sphere {starting_spheres[player]} for {multiworld.player_name[player]}")
            # breakpoint()
    ...
    # Hint point items fund both item and location hints. Each configured value is
    # the number of hints that owner should receive, not a raw point total.
    ...
    hints_by_owner = collections.defaultdict(int)
    for configured_name, hint_count in multiworld.worlds[1].options.hint_count.value.items():
    ...
    available_by_sphere = collections.defaultdict(list)
    for location in swap_out_locations:
        if location in location_sphere:
            available_by_sphere[location_sphere[location]].append(location)
    for locations in available_by_sphere.values():
    ...
    def apportion(weights, total):
        ...
        order = sorted(weights, key=lambda key: (exact[key] - result[key], weights[key], str(key)), reverse=True)
        for key in order[:total - sum(result.values())]:
            result[key] += 1
        return result

    # A point item may occupy any game's location, provided the owner has at least
    # one location in that sphere. Weight by inverse sphere count so each owner's
    # items are spread evenly over their eligible spheres.
    locations_by_owner_sphere = collections.defaultdict(list)
    funded_owners = {owner for owner, hint_count in hints_by_owner.items() if hint_count > 0}
    for sphere_index, candidates in available_by_sphere.items():
        ...
        for owner, count in sorted(counts.items()):
            locations_by_owner_sphere[owner, sphere_index].extend(
                candidates[candidate_index:candidate_index + count])
            candidate_index += count

    placed_locations_by_owner = collections.defaultdict(list)
    for (owner, sphere_index), locations in locations_by_owner_sphere.items():
        placed_locations_by_owner[owner].extend((sphere_index, location) for location in locations)
...
def _move_one_item_to_earlier_sphere(multiworld, spheres, sphere_index, location, merge_jigsaw=False):
    """Move one item to the latest usable location in an earlier sphere."""
    item = location.item
    for earlier_index in range(sphere_index - 1, -1, -1):
        for new_location in spheres[earlier_index]:
            if (merge_jigsaw and new_location.item and item and new_location.player == location.player and
                    multiworld.game[location.player] == "Jigsaw" and "Puzzle Piece" in item.name and
                    "Puzzle Piece" in new_location.item.name):
                new_pieces = int(new_location.item.name.split(" ")[0]) + int(item.name.split(" ")[0])
                new_location.item = multiworld.worlds[location.player].create_item(f"{new_pieces} Puzzle Pieces")
                new_location.item.location = new_location
                location.item = None
                logging.info(f"Merged puzzle pieces into {new_pieces} piece item in sphere {earlier_index + 1}")
                return True
            if (new_location.item is None and new_location.player == location.player and
                    new_location.item_rule(item) and new_location.progress_type != LocationProgressType.EXCLUDED):
                new_location.item = item
                item.location = new_location
                location.item = None
                return True

    logging.info(f"Pushing {item} to start inventory for {multiworld.player_name[item.player]}")
    multiworld.push_precollected(item)
    location.item = None
    return True
...
def compress_spheres(multiworld, max_sphere):
    ...
    while True:
        spheres = gen_spheres()
        if len(spheres) <= max_sphere:
            break
        i += 1
        logging.info(f"compress sphere loop {i}. Number of spheres: {len(spheres)}")
        candidate_spheres = []
        for sphere_index in range(max_sphere, len(spheres)):
            candidates_by_game = collections.defaultdict(list)
            for location in spheres[sphere_index]:
                if (swappable(multiworld, location, within_local=True) and location.item and
                        location.item.advancement):
                    candidates_by_game[location.item.player].append(location)
            candidate_spheres.extend((len(candidates), sphere_index, candidates)
                                     for candidates in candidates_by_game.values())
        if not candidate_spheres:
            break
        _, sphere_index, candidates = min(candidate_spheres,
                                          key=lambda candidate: (candidate[0], -candidate[1]))
        _move_one_item_to_earlier_sphere(multiworld, spheres, sphere_index,
                                         multiworld.random.choice(candidates))
